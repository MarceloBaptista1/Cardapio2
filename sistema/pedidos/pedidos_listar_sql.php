    ...
    try {
        ...
        $sql_cliente = "
                            ...
                            ORDER BY
                                CLIENTES.nome_cliente
                       ";
        $sql_cliente_q = mysqli_query($conn, $sql_cliente);
        if (!$sql_cliente_q) {
            throw new Exception("Erro ao recuperar dados dos clientes");
        }

        while ($sql_cliente_a = mysqli_fetch_assoc($sql_cliente_q)) {
            $array_clientes[] = $sql_cliente_a;
        }

        

    } catch (Exception $e) {
        echo "<span class='badge badge-danger'>".$e->getMessage()."</span>";
    }
